    ...
    <div class="p-6">
        <div class="bg-green-200 w-56 flex justify-center items-center mb-4 rounded-xl hover:scale-100 duration-200 ease-in-out">
            <a href="{{ route('BlackAndWhite.create') }}">
                <h1 class="text-lg font-bold p-3">اضافة صوره جديدة</h1>
            </a>
        </div>
                ...
                @foreach($photos as $photo)
                    <tr class="text-gray-800 text-right">
                        <td class="px-4 py-2 border-b">
                            ...
                            <form action="{{ route('Update.ViewPhotos', $photo->id) }}">
                                @csrf
                                <button type="submit" class="text-blue-500 hover:underline">تعديل</button>
                            </form>
                        </td>
                        <td class="px-4 py-2 border-b">{{ $photo->created_at->format('Y-m-d') }}</td>
                        <td class="px-4 py-2 border-b">
                            @if($photo->image)
                                <img src="{{ asset('public/storage/' . $photo->image) }}" class="h-16 w-16 object-cover rounded" alt="غلاف">
                            @else
                                لا يوجد
                            @endif
                        </td>
                        <td class="px-4 py-2 border-b">{{ Str::limit($photo->description, 50) }}</td>
                        <td class="px-4 py-2 border-b">{{ $photo->Title }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
