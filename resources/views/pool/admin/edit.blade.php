    ...
    <div class="max-w-2xl mx-auto mt-10 bg-white p-6 rounded shadow text-right">
        ...
        <form action="{{ route('pool-update', $pool->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input dir="rtl" value="{{ $pool->title }}" type="text" id="title" name="title"
            ...
            <div class="mb-4">
                ...
                <textarea dir="rtl" id="description" name="description" rows="4"
                          ...
                          required>{{ $pool->description }}</textarea>
            </div>

            @if($pool->cover_url)
                <div class="mb-2">
                    <label class="block mb-1 font-medium">الملف الحالي:</label>
                    <a href="{{ asset('storage/' . $pool->cover_url) }}" target="_blank" class="text-blue-600 underline">عرض الملف</a>
                </div>
            @endif
            ...
            @if($pool->image)
                <div class="mb-2">
                    ...
                    <img src="{{ asset('public/storage/' . $pool->image) }}" class="h-24 rounded">
                </div>
            @endif
