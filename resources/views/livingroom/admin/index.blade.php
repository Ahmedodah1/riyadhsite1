                ...
                @foreach($laundryrooms as $laundryroom)
                    <tr class="text-gray-800 text-right">
                        <td class="px-4 py-2 border-b">
                            ...
                            <form action="{{ route('livingroom-edit-view', $livingroom->id) }}">
                                @csrf
                                <button type="submit" class="text-blue-500 hover:underline">تعديل</button>
                            </form>
                        </td>
                        <td class="px-4 py-2 border-b">{{ $livingroom->created_at->format('Y-m-d') }}</td>
                        <td class="px-4 py-2 border-b">
                            @if($livingroom->image)
                                <img src="{{ asset('public/storage/' . $livingroom->image) }}" class="h-16 w-16 object-cover rounded" alt="غلاف">
                            @else
                                لا يوجد
                            @endif
                        </td>
                        <td class="px-4 py-2 border-b">{{ Str::limit($livingroom->description, 50) }}</td>
                        <td class="px-4 py-2 border-b">{{ $livingroom->title }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
