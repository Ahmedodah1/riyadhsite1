                ...
                @foreach($chambers as $chamber)
                    <tr class="text-gray-800 text-right">
                        <td class="px-4 py-2 border-b">
                            ...
                            <form action="{{ route('chamber-edit-view', $chamber->id) }}">
                                @csrf
                                <button type="submit" class="text-blue-500 hover:underline">تعديل</button>
                            </form>
                        </td>
                        <td class="px-4 py-2 border-b">{{ $chamber->created_at->format('Y-m-d') }}</td>
                        <td class="px-4 py-2 border-b">
                            @if($chamber->image)
                                <img src="{{ asset('public/storage/' . $chamber->image) }}" class="h-16 w-16 object-cover rounded" alt="غلاف">
                            @else
                                لا يوجد
                            @endif
                        </td>
                        <td class="px-4 py-2 border-b">{{ Str::limit($chamber->description, 50) }}</td>
                        <td class="px-4 py-2 border-b">{{ $chamber->title }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
