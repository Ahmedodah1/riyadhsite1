    ...
    <div class="max-w-2xl mx-auto mt-10 bg-white p-6 rounded shadow text-right">
        ...
        <form action="{{ route('warehouse-update', $warehouse->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input dir="rtl" value="{{ $warehouse->title }}" type="text" id="title" name="title"
            ...
            @if($warehouse->cover_url)
                <div class="mb-2">
                    <label class="block mb-1 font-medium">الملف الحالي:</label>
                    <a href="{{ asset('storage/' . $warehouse->cover_url) }}" target="_blank" class="text-blue-600 underline">عرض الملف</a>
                </div>
            @endif
            ...
            @if($warehouse->image)
                <div class="mb-2">
                    ...
                    <img src="{{ asset('public/storage/' . $warehouse->image) }}" class="h-24 rounded">
                </div>
            @endif
