                ...
                @foreach($warehouses as $warehouse)
                    <tr class="text-gray-800 text-right">
                        <td class="px-4 py-2 border-b">
                            ...
                            <form action="{{ route('warehouse-edit-view', $warehouse->id) }}">
                                @csrf
                                <button type="submit" class="text-blue-500 hover:underline">تعديل</button>
                            </form>
                        </td>
                        <td class="px-4 py-2 border-b">{{ $warehouse->created_at->format('Y-m-d') }}</td>
                        <td class="px-4 py-2 border-b">
                            @if($warehouse->image)
                                <img src="{{ asset('public/storage/' . $warehouse->image) }}" class="h-16 w-16 object-cover rounded" alt="غلاف">
                            @else
                                لا يوجد
                            @endif
                        </td>
                        <td class="px-4 py-2 border-b">{{ Str::limit($warehouse->description, 50) }}</td>
                        <td class="px-4 py-2 border-b">{{ $warehouse->title }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
