            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-11">
                @foreach($decors as $decor)
                    <div class="bg-white rounded-2xl border border-gray-200 shadow hover:shadow-md transition duration-300 overflow-hidden">
                        <button @click="open = true; image = @js(asset('public/storage/' . $decor->image))" class="block w-full">
                            <img src="{{ asset('public/storage/' . $decor->image) }}"
                                 alt="{{ $decor->title }}"
                                 class="w-full h-64 object-cover hover:scale-105 transition duration-300" />
                        </button>

                        <div class="p-4 text-right">
                            <h2 class="text-lg font-semibold text-gray-900 truncate">{{ $decor->Title }}</h2>
                            <p class="text-sm text-gray-600 mt-1">{{ $decor->description }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
