...
@section('content')
    <div class="p-6">
        <div class="bg-green-200 w-56 flex justify-center items-center mb-4 rounded-xl hover:scale-100 duration-200 ease-in-out">
            <a href="{{ route('Decor-create-view') }}">
                ...
                @foreach($decors as $decor)
                    <tr class="text-gray-800 text-right">
                        <td class="px-4 py-2 border-b">
                            <form action="{{ route('Decor.delete', $decor->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من الحذف؟');">
                                @csrf
                                <button type="submit" class="text-red-500 hover:underline">حذف</button>
                            </form>
                            <form action="{{ route('Decor-update-view', $decor->id) }}">
                                @csrf
                                <button type="submit" class="text-blue-500 hover:underline">تعديل</button>
                            </form>
                        </td>
                        <td class="px-4 py-2 border-b">{{ $decor->created_at->format('Y-m-d') }}</td>
                        <td class="px-4 py-2 border-b">
                            @if($decor->image)
                                <img src="{{ asset('public/storage/' . $decor->image) }}" class="h-16 w-16 object-cover rounded" alt="غلاف">
                            @else
                                لا يوجد
                            @endif
                        </td>
                        <td class="px-4 py-2 border-b">{{ Str::limit($decor->description, 50) }}</td>
                        <td class="px-4 py-2 border-b">{{ $decor->title }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
