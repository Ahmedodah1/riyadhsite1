                ...
                @foreach($gyms as $gym)
                    <tr class="text-gray-800 text-right">
                        <td class="px-4 py-2 border-b">
                            ...
                            <form action="{{ route('gym-edit-view', $gym->id) }}">
                                @csrf
                                <button type="submit" class="text-blue-500 hover:underline">تعديل</button>
                            </form>
                        </td>
                        <td class="px-4 py-2 border-b">{{ $gym->created_at->format('Y-m-d') }}</td>
                        <td class="px-4 py-2 border-b">
                            @if($gym->image)
                                <img src="{{ asset('public/storage/' . $gym->image) }}" class="h-16 w-16 object-cover rounded" alt="غلاف">
                            @else
                                لا يوجد
                            @endif
                        </td>
                        <td class="px-4 py-2 border-b">{{ Str::limit($gym->description, 50) }}</td>
                        <td class="px-4 py-2 border-b">{{ $gym->title }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
