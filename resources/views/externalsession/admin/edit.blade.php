...
@section('content')
    <div class="max-w-2xl mx-auto mt-10 bg-white p-6 rounded shadow text-right">
        ...
        <form action="{{ route('externalsession-update', $externalsession->id) }}" method="POST" enctype="multipart/form-data">
            @csrf

            <div class="mb-4">
                <label for="title" class="block mb-1 font-medium">عنوان الصورة</label>
                <input dir="rtl" value="{{ $externalsession->title }}" type="text" id="title" name="title"
            ...
            <div class="mb-4">
                ...
                <textarea dir="rtl" id="description" name="description" rows="4"
                          ...
                          required>{{ $externalsession->description }}</textarea>
            </div>

            @if($externalsession->cover_url)
                <div class="mb-2">
                    <label class="block mb-1 font-medium">الملف الحالي:</label>
                    <a href="{{ asset('storage/' . $externalsession->cover_url) }}" target="_blank" class="text-blue-600 underline">عرض الملف</a>
                </div>
            @endif
            ...
            @if($externalsession->image)
                <div class="mb-2">
                    ...
                    <img src="{{ asset('public/storage/' . $externalsession->image) }}" class="h-24 rounded">
                </div>
            @endif
