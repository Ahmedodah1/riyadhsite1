                ...
                @foreach($externalsessions as $externalsession)
                    <tr class="text-gray-800 text-right">
                        <td class="px-4 py-2 border-b">
                            <form action="{{ route('externalsession-delete', $externalsession->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من الحذف؟');">
                                @csrf
                                <button type="submit" class="text-red-500 hover:underline">حذف</button>
                            </form>
                            <form action="{{ route('externalsession-edit-view', $externalsession->id) }}">
                                @csrf
                                <button type="submit" class="text-blue-500 hover:underline">تعديل</button>
                            </form>
                        </td>
                        <td class="px-4 py-2 border-b">{{ $externalsession->created_at->format('Y-m-d') }}</td>
                        <td class="px-4 py-2 border-b">
                            @if($externalsession->image)
                                <img src="{{ asset('public/storage/' . $externalsession->image) }}" class="h-16 w-16 object-cover rounded" alt="غلاف">
                            @else
                                لا يوجد
                            @endif
                        </td>
                        <td class="px-4 py-2 border-b">{{ Str::limit($externalsession->description, 50) }}</td>
                        <td class="px-4 py-2 border-b">{{ $externalsession->title }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
